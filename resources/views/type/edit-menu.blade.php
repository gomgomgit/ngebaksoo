...
<div x-data="init()">
  ...
  <div
    ...
    >
      <form method="POST" action="/admin/type/update-menu/{{$menu->id}}" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      <div class="mt-4 mb-6">
          <div>
            <label class="block mt-4 text-sm">
                ...
                >
                  @foreach ($types as $type)
                    <option {{$menu->type_id == $type->id ? 'selected' : ''}} value="{{$type->id}}">{{$type->name}}</option>
                  @endforeach
                </select>
              </label>
          </div>
            ...
            <div class="w-full">
              <div>
                  <label class="block text-sm">
                    ...
                    <input name="name" value="{{$menu->name}}" class="text-gray-700 block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" placeholder="Nama Menu">
                  </label>
              </div>
              <div class="mt-4">
                <label class="block text-sm">
...
<script>
  ...
  function init() {
    ...
    return {
      imageTypeUrl: "{{$menu->image ? Storage::url($menu->image) : asset('images/no-image.png')}}",
